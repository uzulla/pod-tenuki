...
- 文字起こし処理（`Waiting for transcription to complete...` と表示されている間）はかなりの時間がかかります。音声の長さにもよりますが、1時間の音声で20-30分程度の処理時間を見込んでください。
- 処理中はコンソールに進捗状況が表示され、Google Cloud APIで処理が行われています。
- 処理中に中断しても、すでにGCS上にアップロードされたファイルはライフサイクルポリシーに従って削除されます。
...
- 連結されたMP3ファイル：デフォルトで `./output` ディレクトリに作成
- 連結ファイルから生成される出力ファイル：上記と同様

注意：
- デフォルトですべての出力ファイルは `./output` ディレクトリに保存されます
- `--output-dir` オプションを使用して、別の出力ディレクトリを指定することも可能です
